...
def undo_php_mt_reload_nm1(Smm1, Snm1, Snmm, flavour):
    # define twist_php(m,u,v)  (m ^ (mixBits(u,v)>>1) ^ ((uint32_t)(-(int32_t)(loBit(u))) & 0x9908b0dfU))
    # m S000
    # u S227
    # v S228
    ...
    S0_1_30 = (X << 1)
    for S0_0 in range(2):
        for S0_31 in range(2):
            if flavour == MT_RAND_MT19937 and s22X_0 != S0_0:
                continue
            S0 = S0_0 | S0_31 << 31 | S0_1_30

            # Check if the guessed seed yields S000 as its first scrambled state
            rand = undo_php_mt_reload(S0, Snmm, 0, flavour)
            if not rand:
                continue
            state = php_mt_initialize(rand)
            php_mt_reload(state, flavour)

            if not (Smm1 == state[M-1]):
                continue
...
def main(_Rmm1, _Rnm1, _Rnmm, flavour):
    # Both were >> 1, so the leftmost byte is unknown
    _Rmm1 <<= 1
    _Rnm1 <<= 1
    _Rnmm <<= 1

    for Rmm1_0 in range(2):
        for Rnm1_0 in range(2):
            for Rnmm_0 in range(2):
                Rmm1 = _Rmm1 | Rmm1_0
                Rnm1 = _Rnm1 | Rnm1_0
                Rnmm = _Rnmm | Rnmm_0
                Smm1 = undo_php_mt_rand(Rmm1)
                Snm1 = undo_php_mt_rand(Rnm1)
                Snmm = undo_php_mt_rand(Rnmm)

                seed = undo_php_mt_reload_nm1(Smm1, Snm1, Snmm, flavour)
...
if len(sys.argv) < 5:
    ...
    print('')
    print('Usage:')
    print(f'  {sys.argv[0]} <rand_M-1> <rand_N-1> <rand_N-M> <flavour>')
    print('')
    print('Parameters:')
    print('  rand_M-1: M-1 st random value')
    print('  rand_N-1: N-1 st random value')
    print('  rand_N-M: N-M st random value')
