...
def undo_php_mt_reload_once(S000, S227, offset, flavour):
...
def undo_php_mt_reload_SS(S000, S227, flavour):
    # define twist_php(m,u,v)  (m ^ (mixBits(u,v)>>1) ^ ((uint32_t)(-(int32_t)(loBit(u))) & 0x9908b0dfU))
    # m S000
    # u S227
    # v S228
...
def undo_php_mt_reload(S000, S227, S454, offset, flavour):
    for Sj in undo_php_mt_reload_SS(S000, S227, flavour):
        for Sj227 in undo_php_mt_reload_SS(S227, S454, flavour):
            rand = undo_php_mt_reload_once(Sj, Sj227, offset-N+227+1, flavour)
            if not rand:
                continue
            state = php_mt_initialize(rand)
            php_mt_reload(state, flavour)
            php_mt_reload(state, flavour)
            if not (S000 == state[offset-N]):
                continue

            return rand


def main(_R000, _R227, _R454, offset, flavour):
    ...
    for R000_0 in range(2):
        for R227_0 in range(2):
            for R454_0 in range(2):
                ...
                seed = undo_php_mt_reload(S000, S227, S454, offset, flavour)
                if seed:
                    print(seed)
...
if len(sys.argv) < 6:
    print('Finds out a PHP mt_rand() seed from three outputs of mt_rand()'
          ' separated by 226 other calls.')
    print('')
    print('Usage:')
    print(
        f'  {sys.argv[0]} <rand_n+0> <rand_n+227> <rand_n+454> <n> <flavour>')
    print('')
    print('Parameters:')
    print('  rand_n+0:   First random value')
    print('  rand_n+227: Second random value')
    print('  rand_n+454: Third random value')
    print('    The second value comes 226 mt_rand() calls after the first and')
    print('              the third value after the second')
